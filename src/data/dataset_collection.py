...
class SyntheticDatasetCollection:
    ...
    def process_propensity_intensity_train_f(self, prop_int_treatment, prop_int_history, stabilize=True):
        """
        Generate stabilized weights for SCIP for the train subset
        Args:
            propensity_treatment: Propensity treatment network
            propensity_history: Propensity history network
        """

        prop_hist_train_f = prop_int_history.get_propensity_scores(self.train_f)
        if stabilize:
            prop_treat_train_f = prop_int_treatment.get_propensity_scores(self.train_f)
        else:
            prop_treat_train_f = np.ones(shape=prop_hist_train_f.shape)

        intensity_hist_train_f = prop_int_history.get_intensity_scores(self.train_f)
        if stabilize:
            intensity_treat_train_f = prop_int_treatment.get_intensity_scores(self.train_f)
        else:
            intensity_treat_train_f = np.ones(shape=intensity_hist_train_f.shape)

        self.train_f.data['stabilized_weights'] = (np.prod(prop_treat_train_f / prop_hist_train_f, axis=2) * np.prod(intensity_treat_train_f / intensity_hist_train_f, axis=2))
...
class RealDatasetCollection:
    ...
    def process_propensity_intensity_train_f(self, propensity_treatment, propensity_history, stabilize=True):
        """
        Generate stabilized weights for SCIP for the train subset
        Args:
            propensity_treatment: Propensity treatment network
        ...
        intensity_treat_train_f = propensity_treatment.get_intensity_scores(self.train_f)
        intensity_hist_train_f = propensity_history.get_intensity_scores(self.train_f)
    ...
    def process_data_pretrain(self):
        """
        Used by DynamicCausalPFN pretraining
        """
        pass  
