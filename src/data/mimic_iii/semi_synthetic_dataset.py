...
from src.data.mimic_iii.mimic_pretrain import (
...
class PretrainMIMICDataset(MIMIC3SyntheticDataset):
    """
    Pretraining dataset for DynamicCausalPFN.
    
    This keeps the same processed-data interface as MIMICSyntheticDataset,
    but the raw data comes from mimic_pretrain.py
    """
    def __init__(
            self,
            num_patients:int,
        ...
        if num_datasets == 1:
            raw = generate_pretraining_dataset(
                num_samples=num_patients,
                seq_length=seq_length,
                seed=seed,
            )
        else:
            raw_list = generate_many_pretraining_datasets(
                num_datasets=num_datasets,
                samples_per_dataset=num_patients,
                seq_length=seq_length,
                base_seed=seed,
            )
            raw = self._merge_pretraining_datasets(raw_list)
        

        outcomes = self._ensure_3d(raw['outcomes'])
        vitals = self._ensure_3d(raw['vitals'])
        treatments = self._ensure_3d(raw['treatments'])
        static_features = self._ensure_2d(raw['static_features'])
        sequence_lengths = np.asarray(raw['sequence_lengths']).astype(int)

        n, full_seq_length, _ = outcomes.shape 

        if vitals.shape[0] != n or vitals.shape[1] != full_seq_length:
            raise ValueError(f"vitals shape {vitals.shape} is incompatible with outcomes shape {outcomes.shape}")
        if treatments.shape[0] != n or treatments.shape[1] != full_seq_length:
            raise ValueError(f"treatments shape {treatments.shape} is incompatible with outcomes shape {outcomes.shape}")
        if static_features.shape[0] != n:
            raise ValueError(f"static_features shape {static_features.shape} is incompatible with batch size {n}")
        
        # Convert sequence lengths to number of valid one-step transitions
        if np.max(sequence_lengths) == full_seq_length:
            sequence_lengths = sequence_lengths - 1 
        sequence_lengths= np.clip(sequence_lengths, 1, full_seq_length - 1)

        # Build active mask on transition axis (Length T-1)
        active_entries = np.zeros((n, full_seq_length -1, 1))
        for i in range(n):
            active_entries[i, :int(sequence_lengths[i]), :] = 1.0
        
        # Multilabel treatment interface
        treatments = np.rint(treatments)
        
        # Compute scaling from this dataset if not provided,
        # otherwise reuse train scaling for val.
        if scaling_params is None:
            scaling_params = self._compute_scaling_params_from_raw(
                outcomes=outcomes, 
                vitals=vitals,
                static_features=static_features,
            ...
            scaling_params = self._sanitize_scaling_params(scaling_params)
        
        self.scaling_params = scaling_params

        # Normalize outputs / vitals / static features immediately
        outputs = (
            outcomes[:, 1:, :] - self.scaling_params['output_means']
        ...
        vitals_norm = (
            vitals[:, 1:, :] - self.scaling_params['vitals_means']
        ) / self.scaling_params['vitals_stds']

        if full_seq_length > 2:
            next_vitals_norm = (
                vitals[:, 2:, :] - self.scaling_params['vitals_means']
            ) / self.scaling_params['vitals_stds']
        
        else:
            next_vitals_norm = vitals[:, :0, :]
        
        static_features_norm = (
            static_features - self.scaling_params['static_features_means']
        ) / self.scaling_params['static_features_stds']

        self.data = {
            'sequence_lengths':sequence_lengths,
            'prev_treatments':treatments[:, :-1, :],
            'vitals':vitals_norm,
            'next_vitals':next_vitals_norm,
            'current_treatments':treatments[:, 1:, :],
            'static_features':static_features_norm,
            'active_entries':active_entries,
            'unscaled_outputs': outcomes[:, 1:, :],
            'prev_unscaled_outputs':outcomes[:, :-1, :],
            'outputs':outputs,
        ...
        logger.info(f'Shape of processed {self.subset_name} pretraining MIMIC data: {data_shapes}')
    ...
    @classmethod
    def _compute_scaling_params_from_raw(cls, outcomes, vitals, static_features, sequence_lengths):
        """
        Compute normalisation statistics from raw generated data.
        Uses only active transition steps for outcomes/vitals.
        """
        n, full_seq_length, _ = outcomes.shape

        active_mask = np.zeros((n, full_seq_length - 1), dtype=bool)
        for i in range(n):
            active_mask[i, :int(sequence_lengths[i])] = True 
        ...
        output_stds = cls._safe_std(active_outputs.std(axis=0))

        vitals_means = active_vitals.mean(axis=0)
        vitals_stds = cls._safe_std(active_vitals.std(axis=0))

        static_features_means = static_features.mean(axis=0)
        static_features_stds = cls._safe_std(static_features.std(axis=0))
    ...
    @classmethod
    def _sanitize_scaling_params(cls, scaling_params):
        """
        Make sure stds are non-zero and everything is array-like for broadcasting.
        """
        sanitized = dict(scaling_params)
        sanitized['output_means'] = np.asarray(sanitized['output_means'])
        sanitized['output_stds'] = cls._safe_std(np.asarray(sanitized['output_stds']))
        sanitized['vitals_means'] = np.asarray(sanitized['vitals_means'])
        sanitized['vitals_stds'] = cls._safe_std(np.asarray(sanitized['vitals_stds']))
        sanitized['static_features_means'] = np.asarray(sanitized['static_features_means'])
        sanitized['static_features_stds'] = cls._safe_std(np.asarray(sanitized['static_features_stds']))
        return sanitized
    
    def __len__(self):
        return self.data['active_entries'].shape[0]
    
    def get_scaling_params(self):
        return self.scaling_params

    def process_data(self, scaling_params=None):
        """
        Already processed
        """
...
class PretrainMIMICDatasetCollection(SyntheticDatasetCollection):
    """
    Dataset collection for synthetic MIMIC-style pretraining only.
    """
    ...
    def __init__(
        ...
        super(PretrainMIMICDatasetCollection, self).__init__()

        self.seed=seed 
        np.random.seed(seed)

        self.train_f = PretrainMIMICDataset(
        ...
        self.val_f = PretrainMIMICDataset(
            ...
            treatment_mode=treatment_mode,
            scaling_params=self.train_f.get_scaling_params()
        )
        ...
        # Pretraining collectin does not include downstream mimic test sets.
        # Instantiate MIMIC3SyntheticDatasetCollection separately for downstream evaluation.
