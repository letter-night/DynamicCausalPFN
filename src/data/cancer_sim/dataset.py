...
from src.data.cancer_sim.cancer_simple import TUMOUR_DEATH_THRESHOLD
...
class SyntheticCancerDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.params = generate_params(num_patients, radio_coeff=radio_coeff, window_size=window_size,
                                      lag=lag)
    ...
    def process_data(self, scaling_params):
        ...
        if not self.processed:
            ...
            input_means = mean[['cancer_volume', 'patient_types', 'radio_application']].values.flatten()
            input_stds = std[['cancer_volume', 'patient_types', 'radio_application']].values.flatten()
            ...
            treatments = radio_application[:, :-offset, np.newaxis]
...
class SyntheticCancerDatasetCollection(SyntheticDatasetCollection):
    ...
    def __init__(self,
        ...
        self.train_f = SyntheticCancerDataset(radio_coeff, num_patients['train'], window_size, max_seq_length,
                                              'train', lag=lag, treatment_mode=treatment_mode)

        self.val_f = SyntheticCancerDataset(radio_coeff, num_patients['val'], window_size, max_seq_length, 'val',
                                            lag=lag, treatment_mode=treatment_mode)

        self.test_cf_one_step = SyntheticCancerDataset(radio_coeff, num_patients['test'], window_size,
                                                       max_seq_length, 'test', mode='counterfactual_one_step', lag=lag,
                                                       treatment_mode=treatment_mode
                                                       )

        self.test_cf_treatment_seq = SyntheticCancerDataset(radio_coeff, num_patients['test'], window_size,
...
class PretrainCancerDataset(SyntheticCancerDataset):
    ...
    def _merge_pretraining_datasets(dataset_list):
        ...
        merged['dataset_id'] = np.concatenate(
            [
                np.full(ds['cancer_volume'].shape[0], i)  
    ...
    def process_data(self, scaling_params):
        ...
        if self.treatment_mode == 'multiclass':
            current_treatments = np.eye(2)[treatments[..., 0]]  # [N, T-1, 2]
        ...
        outputs = cancer_volume[:, horizon:, np.newaxis]  # [N, T-1, 1]

        output_means = float(mean['cancer_volume'])
        output_stds = float(std['cancer_volume'])

        active_entries = np.zeros_like(outputs)  
